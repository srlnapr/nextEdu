...
@section('content')
  <button
    ...
    <a href="/jurusans/create">Add Jurusans</a>
  </button>
    ...
    <div class="mb-10 w-full">
      <div class="w-full rounded-sm border border-[#BBBBBB] bg-white p-3">
        ...
        @if ($jurusans->count())
              ...
              @foreach ($jurusans as $jurusan)
                <tr class="px-6 py-3 text-center">
                  ...
                  <td class="flex justify-center border px-6 py-2">
                    ...
                    <a class="mx-2 text-yellow-400" href="/jurusans/{{ $jurusan['id'] }}/edit">
                      Edit
                    </a>
                    <form class="mx-2 text-red-400" action="/jurusans/{{ $jurusan['id'] }}" method="post"
                      class="d-inline">
                      @method('delete')
                      @csrf
          ...
          @else
            <h1 class="mt-2 mb-4 border p-3 text-center text-lg font-light text-primary lg:text-2xl">Tidak ada Jurusan.
            </h1>
        @endif
        </table>
        {{ $jurusans->links() }}
  ...
  <button
    class="btnnn mb-3 rounded-sm border-2 border-black bg-black py-3 px-5 text-white duration-300 ease-out hover:bg-white hover:text-black">
    <a href="/saranpekerjaans/create">Add SaranPekerjaans</a>
  </button>
  <div class="w-full lg:mx-auto">
    <div class="mb-10 w-full">
      <div class="w-full rounded-sm border border-[#BBBBBB] bg-white p-3">
        ...
        @if ($saranpekerjaans->count())
          <table class="mb-3 w-full rounded-xl text-slate-800">
            ...
            <tbody>
              @foreach ($saranpekerjaans as $saranpekerjaan)
                <tr class="px-6 py-3 text-center">
                  <td class="border px-6 py-2">{{ $loop->iteration }}</td>
                  @foreach ($jurusasans as $jurusan)
                    @if ($jurusan['id'] == $saranpekerjaan['jurusan_id'])
                      <td class="border px-6 py-2">{{ $jurusan['jurusans'] }}</td>
                    @endif
                  @endforeach
                  <td class="border px-6 py-2 text-justify">{{ $saranpekerjaan['saranpekerjaan'] }}</td>
                  <td class="flex justify-center border px-6 py-2">
          ...
          @else
            <h1 class="mt-2 mb-4 border p-3 text-center text-lg font-light text-primary lg:text-2xl">Tidak ada SaranPekerjaan.
            </h1>
        @endif
        </table>
        {{ $jurusans->links() }}
